...
const SideBar = () => {
    ...
    return (
        <div className='   lg:h-[96.5vh]  bg-[#3f3f45] lg:w-[5vw]  flex lg:flex-col items-center lg:py-4 p-2 lg:space-y-6 lg:justify-start justify-between lg:rounded-lg lg:rounded-r-none'>
            <div className=" ">
                <Tooltip content={'Chats'} placement='top' animation={'duration-1000'} className='' style='light'  >
                    <NavLink to={'/home'} onClick={()=>{setIsHome(true)}} className={ ` ${isHome?' bg-[#656565]  ':''} transition-all duration-300 lg:hover:bg-[#656565] flex  items-center justify-center lg:w-14 lg:h-14 rounded-full`} >
           ...
           
                {
                    optionsArr.map((ele)=>(
                        ...
                        <NavLink to={`/home/${ele.to}`} onClick={()=>{setIsHome(false)}} className={ ({isActive})=>` ${isActive?` ${!ele.img?"bg-[#656565]":" lg:bg-[#656565]  border-[#a3a3a3]"}  `:''}  transition-all duration-300 lg:hover:bg-[#656565] flex items-center   justify-center    ${!ele.img?"rounded-full":"  lg:p-1 "} lg:h-14 lg:w-14 w-10 rounded-full `}>
    
                           {
                            ele.img?<span className=' lg:h-full lg:w-full h-9 w-9  '>
